...
public class RedisRakingRepository {
    ...
    public void increaseScore(Long productId, int count) {
        ...
        if (!zSet.isExists()) {
            zSet.expire(RedisKeys.PRODUCT_RANKING.getTtl());
        }
    }
    ...
    public List<Long> getTop5ProductsLast3Days() {
        ...
        zops.unionAndStore(d0, Arrays.asList(d1, d2), RedisKeys.PRODUCT_RANKING_3DAYS.getKey());

        // 집계 결과 TTL 설정 (집계 주기에 맞춰 짧게 유지)
        redisTemplate.expire(RedisKeys.PRODUCT_RANKING_3DAYS.getKey(), RedisKeys.PRODUCT_RANKING_3DAYS.getTtl());

        Set<String> ids = redisTemplate.opsForZSet().reverseRange(RedisKeys.PRODUCT_RANKING_3DAYS.getKey(), 0, 4);
    ...
    public void clearRanking() {
        RScoredSortedSet<String> zSet = redissonClient.getScoredSortedSet(RedisKeys.PRODUCT_RANKING.getKey());
    ...
    private String applySuffix(LocalDate date) {
        return RedisKeys.PRODUCT_RANKING.getKey() + ":" + date.toString();
    }
}
